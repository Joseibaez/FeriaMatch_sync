...
import { Calendar, LayoutDashboard, CalendarDays, Settings, Building2, ChevronLeft, User, Users, Search, LogOut, Home } from "lucide-react";
...
const AppSidebar = () => {
  ...
  return <Sidebar collapsible="icon" className="hidden md:flex">
      ...
      <SidebarHeader className="border-b border-sidebar-border">
        <Link to="/app" className="flex items-center gap-2 px-2 py-3 min-h-[44px] rounded-md transition-colors bg-sky-300">
      ...
      <SidebarContent className="bg-primary-foreground">
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              {visibleNavItems.map(item => <SidebarMenuItem key={item.title}>
        ...
        {visibleAdminItems.length > 0 && <SidebarGroup>
            <SidebarGroupLabel>Administración</SidebarGroupLabel>
            <SidebarGroupContent>
              <SidebarMenu>
                {visibleAdminItems.map(item => <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild isActive={isActive(item.url)} tooltip={item.title}>
                      <NavLink to={item.url} className="flex items-center gap-2">
                        <item.icon className="h-4 w-4" />
                        <span className="flex-1">{item.title}</span>
                        {!isCollapsed && item.badge && <Badge variant="outline" className="ml-auto text-[10px] px-1.5 py-0 h-5 bg-warning/10 text-warning border-warning/20">
                            {item.badge}
                          </Badge>}
                      </NavLink>
                    </SidebarMenuButton>
        ...
        <SidebarGroup className="mt-auto">
          <SidebarGroupContent>
            <SidebarMenu>
              {visibleSettingsItems.map(item => <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild isActive={isActive(item.url)} tooltip={item.title}>
                    <NavLink to={item.url}>
                      <item.icon className="h-4 w-4" />
                      <span>{item.title}</span>
      ...
      <SidebarFooter className="border-t border-sidebar-border">
        <SidebarMenu>
          {/* User email display */}
          {!isCollapsed && user && <div className="px-2 py-2">
          ...
          {/* Logout button */}
          <SidebarMenuItem>
            <SidebarMenuButton onClick={handleLogout} tooltip="Cerrar sesión" className="text-destructive hover:text-destructive hover:bg-destructive/10">
